...
test.describe('Theme feature', () => {
  test('Create theme sucessfully', async ({ page }) => {
    ...
    await page.fill('input#nameTheme', 'La Musique Française');
    ...
    await test.step('Verify the number of themes', async () => {
      const themeList = await page.$$eval('#themeList ul#listThemeAndQuiz li', (elements) => elements.length);
      expect(themeList).toBe(themeListNb);
    });
    ...
    await test.step('Verify the name of the added theme', async () => {
      ...
      expect(themeNames).toContain('La Musique Française');
    });
  });
});